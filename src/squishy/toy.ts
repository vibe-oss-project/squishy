...
export class SquishyToy {
  ...
  constructor(spec:SquishySpec,model:Awaited<ReturnType<typeof loadSquishy>>,environment:EnvironmentSpec) {
    ...
    this.material=new THREE.MeshPhysicalNodeMaterial({roughness:sticky?.49:.86,metalness:0,clearcoat:sticky?.22:0,clearcoatRoughness:.38});
    this.material.colorNode=sculptPaint(spec.id,model.manifest);
    // Fine matte grain catches the studio light without swimming as it deforms.
    this.material.roughnessNode=mix(float(sticky?.45:.82),float(sticky?.51:.87),mx_noise_float(attribute<'vec3'>('restPosition','vec3').mul(850)).mul(.5).add(.5));
  ...
  setWorld(environment:EnvironmentSpec){this.body.world=new StickyWorld(this.body,{strength:this.spec.adhesion,grip:environment.grip,friction:environment.friction,planes:this.spec.family==='sticky'?ROOM_PLANES:OPEN_PLANES});this.body.wake();}
